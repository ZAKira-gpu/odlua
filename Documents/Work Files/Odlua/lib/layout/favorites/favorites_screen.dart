...
class _FavoritesScreenState extends State<FavoritesScreen>
  ...
  Future<void> _toggleDishFavorite(String dishId, bool isCurrentlyFavorite) async {
  ...
  Future<void> _toggleChefFavorite(String chefId, bool isCurrentlyFavorite) async {
    final user = _auth.currentUser;
    if (user == null) return;
  ...
  Widget _buildFavoriteDishesTab(String userId) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, favoriteSnapshot) {
        ...
        final favoriteDishIds = favoriteSnapshot.data?.docs.map((doc) => doc.id).toList() ?? [];
  ...
  Widget _buildFavoriteChefsTab(String userId) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, favoriteSnapshot) {
        ...
        final favoriteChefIds = favoriteSnapshot.data?.docs.map((doc) => doc.id).toList() ?? [];
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, chefsSnapshot) {
            ...
            final favoriteChefs = chefsSnapshot.data?.docs
                .map((doc) {
                    ...
                    children: [
                      ...
                      FutureBuilder<Map<String, dynamic>?>(
                        ...
                        builder: (context, snapshot) {
                          final chefName = snapshot.data?['displayName'] ?? 'Unknown Chef';
                          return Text(
                            'By $chefName',
                            style: const TextStyle(
                              fontSize: 13,
                      ...
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 12),
                          Text(
                            dish.price == 0 ? 'free'.tr() : '€${dish.price.toStringAsFixed(2)}',
                            style: const TextStyle(
                              fontSize: 14,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 4),
                        Text(
                          '${chef['rating'].toStringAsFixed(1)} (${chef['reviewCount']} reviews)',
                          style: TextStyle(
                            fontSize: 13,
                            color: Colors.grey.shade600,
                          ),
          ...
          if (chef['categories'] != null && (chef['categories'] as List).isNotEmpty)
            Wrap(
              spacing: 8,
              runSpacing: 4,
